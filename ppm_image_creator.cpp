...
int main() {
	...
	auto* image_data = (std::byte*) malloc(SIZE);	//Allocating memory for image of size w: 256 & h: 256 pixels & 3 colour channels
	...
	{ //Image Painter - sets the background and the diagonal line to the array
		for (int row = 0; row < width; row++) {
			for (int col = 0; col < height; col++) {
				...
				//Background
				image_data[pixel*3] = (std::byte) (row % 256); //red
				image_data[pixel*3 + 1] = (std::byte) (col % 256); //green
				image_data[pixel*3 + 2] = (std::byte) (col % 256); //blue

				//Black line - comment this out to remove diagonal black line
				image_data[col * 3 + col*(256 * 3)] = (std::byte) 0;
				image_data[col * 3 + col*(256 * 3) + 1] = (std::byte) 0;
				image_data[col * 3 + col*(256 * 3) + 2] = (std::byte) 0;
	...
	{ //Image Body - outputs image_data array to the .ppm file, creating the image
		for (int x = 0; x < SIZE; x++) {
			int value = (int) image_data[x];		//Sets value as an integer, not a character value
